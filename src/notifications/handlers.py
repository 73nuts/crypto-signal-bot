...
async def handle_alert_notification(event: AlertDetectedEvent):
    ...
    if breaker.is_open():
        ...
        logger.info("[EventHandler] CircuitBreaker OPEN, routing to Digest")
        for lang in ["zh", "en"]:
            _add_to_digest(
                "alert",
                event.symbol,
...
async def handle_spread_notification(event: SpreadDetectedEvent):
    ...
    breaker = _get_circuit_breaker()
    if breaker.is_open():
        logger.info("[EventHandler] CircuitBreaker OPEN, routing spread to Digest")
        for lang in ["zh", "en"]:
            _add_to_digest(
                "spread",
                event.symbol,
...
async def handle_orderbook_notification(event: OrderbookImbalanceEvent):
    ...
    breaker = _get_circuit_breaker()
    if breaker.is_open():
        logger.info("[EventHandler] CircuitBreaker OPEN, routing orderbook to Digest")
        for lang in ["zh", "en"]:
            _add_to_digest(
                "orderbook",
                event.symbol,
